...
export async function POST(request: NextRequest) {
  try {
    ...
    const { bounds, zoom, timeOfDay = "afternoon", userType = "pedestrian" } = body;
    ...
    if (north <= south || east <= west) {
      return NextResponse.json(
        createAPIResponse(false, null, "Invalid bounds"),
        { status: 400 }
      );
    }
    ...
    // Generate heatmap data using the main function
    const heatmapData = await generateHeatmapData(bounds, zoom, timeOfDay, userType);
    ...
    const combinedData = [
      ...realData.map((point: any) => ({
        latitude: point.latitude,
        longitude: point.longitude,
        score: point.overallScore || point.score,
